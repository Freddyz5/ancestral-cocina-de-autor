/*
  ...
  razonar —y probar— sin abrir un navegador. Copiado sin cambios desde
  portafolio: es lógica pura, no depende de marca ni de idioma.
*/
import type {
  ChoiceGroup,
  CounterItem,
...
function toggleLine(item: ToggleItem, phaseId: string): QuoteLine {
  return { id: item.id, phaseId, name: item.name, quantity: 1, amount: item.price };
}

function counterLine(item: CounterItem, phaseId: string, count: number): QuoteLine {
  return {
    id: item.id,
    phaseId,
    name: item.name,
    quantity: count,
    amount: count * item.pricePerUnit,
  };
}

function choiceLine(group: ChoiceGroup, phaseId: string, optionId: string): QuoteLine | null {
  const option = group.options.find((candidate) => candidate.id === optionId);
  if (!option) return null;
  return { id: option.id, phaseId, name: option.name, quantity: 1, amount: option.price };
}

function lineForItem(item: QuoteItem, phase: Phase, selection: Selection): QuoteLine | null {
  if (item.kind === 'toggle') {
    return selection.toggles[item.id] ? toggleLine(item, phase.id) : null;
  }
  if (item.kind === 'counter') {
    const count = selection.counters[item.id] ?? 0;
    return count > 0 ? counterLine(item, phase.id, count) : null;
  }
  return choiceLine(item, phase.id, selection.choices[item.id]);
}
...
export function computeQuote(
  ...
  for (const phase of data.phases) {
    ...
    for (const item of phase.items) {
      ...
      const line = lineForItem(item, phase, selection);
      if (line) phaseLines.push(line);
    }

    const amount = phaseLines.reduce((sum, line) => sum + line.amount, 0);
    byPhase.push({ phaseId: phase.id, amount, lines: phaseLines });
